...
const CATEGORY_WORDS: Record<Category, string[]> = {
  ...
  study: ['study', 'school', 'learn', 'learning'],
  health: ['health', 'fitness', 'gym', 'walk'],
  other: ['other'],
}
...
function findTodo(todos: Todo[], query: string) {
...
function removeTaskInstruction(text: string) {
  return text
    .replace(
...
export function TodoChatbot({
  ...
  const handleCommand = (rawInput: string) => {
    ...
    if (
      ...
      const title = removeTaskInstruction(text)
      ...
      {open && (
        <>
          ...
          <motion.section
            ...
            className="fixed inset-x-3 bottom-3 z-[60] mx-auto flex h-[min(680px,calc(100vh-1.5rem))] max-w-[560px] flex-col overflow-hidden rounded-[30px] border border-[var(--border)] bg-[var(--surface)] shadow-[0_30px_80px_rgba(38,31,45,0.25)]"
            role="dialog"
            aria-modal="true"
            aria-label={t(
              'assistant.title',
          ...
          >
            <header className="flex items-center justify-between border-b border-[var(--border)] px-5 py-4">
              <div className="flex items-center gap-3">
                <span className="grid size-11 place-items-center rounded-[16px] bg-[color-mix(in_srgb,var(--accent)_16%,var(--surface-muted))] text-[var(--accent)]">
                  <Sparkles size={21} />
                </span>
              ...
              <button
                ...
                className="grid size-10 place-items-center rounded-[14px] text-[var(--muted)] transition hover:bg-[var(--surface-muted)] hover:text-[var(--text)]"
              >
                <X size={19} />
              </button>
            </header>
              ...
              {messages.map((message) => (
                ...
                >
                  {message.role === 'assistant' && (
                    <span className="mt-1 grid size-8 shrink-0 place-items-center rounded-[11px] bg-[var(--surface-muted)] text-[var(--accent)]">
                      <Bot size={16} />
                    </span>
                  )}

                  <div
                    className={`max-w-[82%] whitespace-pre-line rounded-[20px] px-4 py-3 text-sm font-semibold leading-5 ${
                      message.role === 'user'
            ...
            <div className="hide-scrollbar flex gap-2 overflow-x-auto px-4 pb-3 sm:px-5">
              {[
                ...
                <button
                  ...
                  className="shrink-0 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-bold text-[var(--muted)] transition hover:border-[var(--accent)] hover:text-[var(--text)]"
                >
                  {suggestion}
                </button>
              ))}
            ...
            >
              <div className="flex items-center gap-2 rounded-[21px] border border-[var(--border)] bg-[var(--surface-muted)] p-1.5 pl-4">
                <input
                  value={input}
                  onChange={(event) =>
                  ...
                  className="min-w-0 flex-1 bg-transparent py-2.5 text-sm font-bold text-[var(--text)] outline-none placeholder:text-[var(--muted)]"
                />

                <button
                  type="submit"
                  disabled={!input.trim()}
                  className="grid size-10 shrink-0 place-items-center rounded-[15px] bg-[var(--accent)] text-[#302738] transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <ArrowUp
                    size={19}
                    strokeWidth={2.8}
